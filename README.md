...
```text
E:\Projects\project-launcher-menu
```
...
```text
E:\Projects\project-launcher-menu\bat\Project Launcher Menu.exe
```

5. 如果只想运行 BAT 菜单，可以打开：

```text
E:\Projects\project-launcher-menu\bat\runtime\启动菜单.bat
```

6. 如果需要重新生成菜单，运行：

```powershell
cd "E:\Projects\project-launcher-menu\bat"
...
1. 双击或固定打开 `E:\Projects\project-launcher-menu\bat\Project Launcher Menu.exe`。
2. 在打开的菜单中查看编号。
3. 输入对应数字，例如 `7`。
...
1. 在 `E:\Projects\project-launcher-menu\bat\launchers` 下新增一个启动文件，例如：
...
1. 打开 `E:\Projects\project-launcher-menu\bat\launchers`。
2. 删除、移动或重命名对应的 `NN-name.bat`。
3. 运行 `python generate_bats.py`。
4. 检查 `E:\Projects\project-launcher-menu\bat\runtime\启动菜单.bat` 是否已经更新。
...
v0.2.6 (2026-06-11)
- 🔄 本地项目目录从 `E:\Projects\launchers` 重命名为 `E:\Projects\project-launcher-menu`。
- 🔧 重新生成 `bat/runtime/启动菜单.bat`，修复菜单内启动项仍指向旧目录的问题。
- 📋 更新 README 中的安装、使用和维护路径说明。
v0.2.5 (2026-06-11)
- 🔧 修复 `Project Launcher Menu.exe` 仍调用旧路径 `bat/启动菜单.bat` 的问题。
- 🔧 修复 `Project Launcher Menu.spec` 移入 `packaging/` 后相对路径失效的问题。
- ⚡ 重新打包 EXE，使其按新结构调用 `bat/runtime/启动菜单.bat`。
v0.2.4 (2026-06-11)
