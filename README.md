...
一个 Windows 本地项目启动菜单，把分散在不同目录里的 BAT/Python 工具整合成一个编号入口，并提供可固定到任务栏的 `Project Launcher Menu.exe` 图形入口。

## 1. 一句话定位

Project Launcher Menu 是一个轻量级 Windows 启动器：用 `Project Launcher Menu.exe` / `启动菜单.bat` 汇总多个本地项目，输入编号即可启动对应工具。
...
- 打开一个菜单，输入编号即可启动对应项目。
- 可以把 `Project Launcher Menu.exe` 固定到任务栏或开始菜单，当作本机工具入口。
...
| 功能 | 解决什么问题 |
| --- | --- |
| 可固定 EXE 入口 | `Project Launcher Menu.exe` 可以固定到底部任务栏，减少每次找 BAT 的麻烦。 |
| 编号启动菜单 | 把多个项目统一放进一个命令行菜单，输入数字即可启动。 |
| 独立项目 BAT | 每个项目一个启动脚本，避免主菜单里堆复杂命令。 |
| ASCII 菜单文本 | 避免 Windows CMD 中文编码导致菜单乱码或命令被误执行。 |
| 一键重新生成菜单 | `generate_bats.py` 自动扫描 `NN-name.bat`，重建主菜单。 |
...
1. 双击或固定打开 `bat/Project Launcher Menu.exe`。
2. 在打开的菜单中查看编号。
3. 输入对应数字，例如 `7`。
...
| 工具 | 用途 |
| --- | --- |
| `Project Launcher Menu.exe` | 当前实际使用的固定入口，可以放到底部任务栏。 |
| `Project Launcher Menu.spec` | 当前 EXE 的 PyInstaller 打包配置。 |
| `启动菜单.bat` | 主启动菜单。 |
...
└── bat/
    ...
    ├── Project Launcher Menu.spec # 当前 EXE 打包配置
    ├── 废弃/
    │   ├── 启动菜单.exe          # 早期可用入口
    │   ├── 启动菜单.spec         # 早期入口打包配置
    │   └── project-launcher-icon.ico # 早期图标资源
    ├── 启动菜单.bat             # 主菜单入口
    ├── 01-video-sub-md.bat
...
Q: 为什么上传 `Project Launcher Menu.exe`?
A: 这是当前实际使用入口，可以固定到任务栏或开始菜单。早期入口已移动到 `bat/废弃/`，旧的 `启动菜单ps.exe`、`启动菜单ps2.exe` 和 build/dist 中间产物仍然忽略。
...
v0.2.3 (2026-06-11) 🔄 当前入口重命名为 `Project Launcher Menu.exe` / `Project Launcher Menu.spec` 📋 将早期 `启动菜单.exe`、`启动菜单.spec`、`project-launcher-icon.ico` 移入 `bat/废弃/` ✅ 保留正式 EXE、SPEC、图标在仓库中，方便固定到任务栏和后续重新打包
v0.2.2 (2026-06-11) ✅ `generate_bats.py` 改为自动扫描 `NN-name.bat` 文件 ✅ 新增项目只需放入类似 `08-my-tool.bat` 后重新生成菜单 ⚡ 主菜单不再依赖手写 PROJECTS 清单，降低维护成本
